...
class MicrophoneClient extends React.Component {
  ...
  setupMicrophone() {
    ...
    self.processRawInterval = setTimeout(function computeVolume() {
      if (self.state.micOn && socket.connected && lastVolumes.length) {
        // calculate average
        self.averageVolume = _.reduce(lastVolumes, function (memo, num) {
          return memo + num;
        }, 0) / lastVolumes.length;
        lastVolumes = [];
  ...
  render() {
    ...
    if (this.state.micOn) {
      buttonAction = React.createElement(
        'a',
        { href: '#', onClick: e => this.turnOff() },
        'TURN MIC OFF'
      );
    } else {
      buttonAction = React.createElement(
        'a',
        { href: '#', onClick: e => this.turnOn() },
        'TURN MIC ON'
      );
    }
