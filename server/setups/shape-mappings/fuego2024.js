...
module.exports = function getShapes() {
  ...
  const all = _.range(0,3000)

  const inv = arr => [].concat(arr).reverse();

  const TIRA_HD = 300;
  const TOTEM_LEDS = 300;
  ...
  // Totems
  const totemL1 = _.range(T4_end, T4_end+TOTEM_LEDS);
  const totemL2 = _.range(T4_end+TOTEM_LEDS, T4_end+TOTEM_LEDS*2);
  const totemL3 = _.range(T4_end+TOTEM_LEDS*4, T4_end+TOTEM_LEDS*5);
  const totemR1 = _.range(T4_end+TOTEM_LEDS*2, T4_end+TOTEM_LEDS*3);
  const totemR2 = _.range(T4_end+TOTEM_LEDS*3, T4_end+TOTEM_LEDS*4);
  const totemR3 = _.range(T4_end+TOTEM_LEDS*5, T4_end+TOTEM_LEDS*6);

  const totems = _.flatten([totemL3, totemL1, totemL2, totemR1, totemR2, totemR3]);

  const totemsExt = _.flatten([totemL1, totemR2]);
  const totemsInt = _.flatten([totemL2, totemR1]);
  const totemsExtExt = _.flatten([totemL3, totemR3]);
  ...
  const allOfIt = _.range(comienzoTira1, 3000); // TODO: revisar por leds muertos en la W
  ...
  const wings = _.flatten([totemL3, totemL1, totemL2, V1L, V2R, totemR1, totemR2, totemR3]);

  const wingsRight = _.flatten([V1R, V2R, totemR1, totemR2, totemR3]);
  const wingsLeft = _.flatten([totemL3, totemL1, totemL2, V1L, V2L]);
  const wingsX = _.flatten([totemL3, totemL1, totemL2, X, totemR1, totemR2, totemR3]);
