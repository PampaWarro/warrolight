...
module.exports = function getShapes(mapping) {
  ...
  const shuffleSegments5 = _.flatten(
    _.shuffle(_.map(_.range(0, 150 / 5), i => _.range(i * 5, (i + 1) * 5)))
  );
  const shuffleSegments10 = _.flatten(
    _.shuffle(_.map(_.range(0, 150 / 10), i => _.range(i * 10, (i + 1) * 10)))
  );
  const shuffleSegments20 = _.flatten(
    _.shuffle(_.map(_.range(0, 7), i => _.range(i * 20, (i + 1) * 20)))
  );
