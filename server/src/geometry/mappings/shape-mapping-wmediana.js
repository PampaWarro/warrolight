...
module.exports = function getShapes(mapping) {
  ...
  const trianguloBottomLeft = _.range(
    comienzoTira2,
    comienzoTira2 + portion / 2 + centroOffsetLeft
  );
  const trianguloBottomRight = _.range(
    comienzoTira3,
    comienzoTira3 + portion / 2 + centroOffsetRight
  );
  ...
  const shuffleSegments5 = _.flatten(
    _.shuffle(_.map(_.range(0, 120), i => _.range(i * 5, (i + 1) * 5)))
  );
  const shuffleSegments10 = _.flatten(
    _.shuffle(_.map(_.range(0, 60), i => _.range(i * 10, (i + 1) * 10)))
  );
  const shuffleSegments20 = _.flatten(
    _.shuffle(_.map(_.range(0, 30), i => _.range(i * 20, (i + 1) * 20)))
  );
  ...
  const allOfIt = _.flatten([
    _.range(comienzoTira1, 150),
    _.range(comienzoTira2, 300),
    _.range(comienzoTira3, 450),
    _.range(comienzoTira4, 600)
  ]);

  // Numeros y letras
  const char_1 = _.range(150, 300);
  const char_2 = _.flatten([
    inv(trianguloBottomBottom),
    trianguloBottomLeft,
    trianguloTopRight,
    trianguloTopTop
  ]);
  const char_3 = _.flatten([
  ...
  const char_a = _.flatten([
    trianguloBottom,
    trianguloTopRight,
    trianguloTopTop,
    inv(_.range(450 - quiebre_arriba - 10, 450 - quiebre_arriba))
  ]);
  const char_o = _.flatten([
    reloj.slice(0, 90 - 20),
    reloj.slice(90 + 20, 270 - 20),
    reloj.slice(270 + 20, 360)
  ]); // El reloj sin el centro
  const char_r = _.flatten([
    trianguloBottomLeft,
    trianguloTop,
    inv(trianguloBottomRight)
  ]);
  const char_w = Warro;
