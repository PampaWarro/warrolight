...
class MetaSchedule {
  constructor() {
    this.schedules = {
      ...
      low : randomSchedule((program, presetName, config) =>
                               config.tags &&
                               config.tags.includes('intensity-low'),
                           baseDuration, randomDuration),
      mid : randomSchedule((program, presetName, config) =>
                               config.tags &&
                               config.tags.includes('intensity-mid'),
                           baseDuration, randomDuration),
      high : randomSchedule((program, presetName, config) =>
                                config.tags &&
...
module.exports = class JoyaMusic extends createMultiProgram
(() => metaSchedule.schedule(), false, 5 * seconds) {
  init() {
    super.init();
    this.masks = {};
    for (const [name, shape] of Object.entries(this.shapeMapping())) {
      const mask = this.masks[name] = new Array(this.numberOfLeds).fill(false);
