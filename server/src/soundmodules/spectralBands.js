...
class SpectralBands {
  constructor(config) {
    ...
    _.forOwn(this._bands, (band, name) => {
      const minBin = Math.ceil(fftbins.getFFTBinForFreq(
        band.freqs[0], config.sampleRate, config.frameSize * 2))
      const maxBin = Math.floor(fftbins.getFFTBinForFreq(
        band.freqs[1], config.sampleRate, config.frameSize * 2))
      band.bins = [
        Math.max(0, minBin),
        Math.min(config.frameSize - 1, maxBin),
      ];
    });
  }
  run(frame, emitter) {
    ...
    frame.allChannels.forEach(channel => {
      ...
      _.forOwn(that._bands, (bandConfig, bandName) => {
        const subspectrum = channel.absolutefft.slice(
          bandConfig.bins[0], bandConfig.bins[1]);
